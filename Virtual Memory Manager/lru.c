...
void lruFixUp(int index) {
    for (int i = 0; i < PAGE_SIZE; i++) {
        if (pt->pages[i] == index) {//check if found index
            pt->pages[i] = -1; //if so, victimize/invalidate that index
        }
    }
}

int getFrameList() { //helps keep trck of free frames
    ...
    lruFixUp(frameIndex); // special lru fixup case for modifacation from 256 to 128
